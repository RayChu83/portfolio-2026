 ...
 *
 * Not shared with the static build, which has no tilt to swing: `useTilt`
 * returns before attaching anything under the preference, so there would be
 * nothing to multiply these by.
 */
const SWING = { rotateX: -8, rotateY: 12 };

/**
 * A media query that is true everywhere, holding the conditions object below
 * open — see the `media.add` call for what it is guarding against.
 */
const ALWAYS = "all";
...
/**
 * The hero as it is meant to be seen: pinned, scrubbed, and reading itself out
 * beat by beat. Mounted only for visitors who have not asked for less motion —
 * `HeroHeadshot` is the switch that decides, and `HeroStatic` is what everyone
 * else gets.
 *
 * `mapData` arrives as a prop rather than being fetched here, because both
 * builds need the same geometry and the switch is the one place that knows
 * which density to ask for. It is still nullable: the import resolves a beat
 * after mount, and the hero's first paint must not wait on it — so the reveal
 * gates on it below and the markup renders without it.
 */
export default function HeroAnimated({ mapData }: { mapData: MapData | null }) {
  ...
  useGSAP(
    () => {
      ...
      // from the switch above, and the reveal flies the photograph at the
      // marker position it carries. Nothing here can be built until both
      // exist.
      ...
      //
      // A browser that cannot parse COMPACT reports it as simply not
      // matching, which lands such a visitor on the full build — the behaviour
      // they had before any of this existed — rather than on no build at all,
      // which is where a pair of complementary queries would strand them.
      //
      ...
      //
      // `always` is what makes any of the above happen at all, and it is not
      // decoration. `matchMedia.add` only invokes the callback when at least
      // one of the object's queries currently matches — `active && func(...)`
      // in GSAP's own source — and `_onMediaChange` gates a rebuild on the
      // same `anyMatch`. COMPACT is by design the one thing a desktop does
      // *not* match, so an object holding only COMPACT builds the reveal for
      // phones and tablets and builds nothing whatsoever for everyone else.
      // The old single-component hero was carrying `motion: MOTION` alongside
      // it, which was always true here and quietly kept the object active;
      // reduced motion moved up to `HeroHeadshot` and the condition went with
      // it, taking the desktop reveal along by accident.
      //
      // So the guard is restated as a query that cannot fail. `"all"` matches
      // every medium by definition, and being a real query rather than GSAP's
      // special-cased `all` *key* means it registers a listener like any other
      // and is re-read on every media change — nothing here depends on the
      // undocumented half of that API. `compact` is still the only condition
      // read below, and it still narrows.
      media.add({ always: ALWAYS, compact: COMPACT }, (context) => {
        ...
        reveal.fromTo(
          ...
          compact
            ? { scale: 1, x: 0, y: 0 }
            : { scale: 1, borderRadius: PHOTO_RADIUS_REST, x: 0, y: 0 },
          {
            scale: PHOTO_SHRINK,
  ...
  return (
    <>
      {/* `fixed`, so the two of them sit in the corners of the screen. The
          ...
          paragraphs to draw, and draws them.

          None of which has any business existing without the pin, which is
          why it is here and not in `HeroStatic`: `fixed` elements never scroll
          away on their own — the exit tweens and `syncLabels` are what take
      ...
      >
        ...
        >
          ...
          >
            ...
            {/* The wrapper is always rendered and the map inside it is not.
                That split is the point: `mapRef` has to exist for the timeline
                to find, and the layer has to be there for the measurements
                around it to resolve against, but neither needs a single dot
                drawn to be true. */}
        ...
        {/* The third corner, and the only one of the three that belongs to the
            ...
            All of which is a formula about a pinned, centred stage, with the
            flow space under it reserved by a refresh handler this build
            installs — so it belongs to this build and nowhere else.
            `HeroStatic` has no pin to write a formula about, and puts the same
            words in ordinary flow instead. */}
        <p
          ref={placeRef}
        ...
        {/* Perspective lives on a wrapper that is exactly the scene's box, so
            ...
            `absolute` with `inset-y-0`, not the flex-stretched `h-full` this
            used to carry — and it has to be. The stage's own height is a flex
            item's *main* size, handed down by the column above it via
            `flex-1` rather than stated as a CSS `height`. Chromium tracks that
            as a definite size for the stage's own box, but not reliably for a
            *flex-item child* computing a percentage height against it once an
            `aspect-ratio` is also in play — the two-level nesting, plus the
            transferred size the ratio needs, is exactly where that definite
            size stops propagating, and the child collapses to 0×0 with no
            error anywhere. It cost nothing on tall, narrow screens, where the
            stage has height to spare either way, which is why it went
            unnoticed until a wide, short window (a laptop's actual browser
            height, not just its width) put a small number through the buggy
            path.

            Ordinary top/bottom insets on an absolutely positioned box are a
            different, older piece of the spec — height falls out of the two
            offsets against the containing block's padding box, the same
            mechanism the map layer beside it already uses with `inset-0` — and
            that resolves against this exact stage correctly regardless of
            which axis produced its size. `left-1/2 -translate-x-1/2` recovers
            the horizontal centring `justify-center` on the stage used to give
        ...
        >
          ...
          >
            ...
            <Image
              ...
              priority
              // No `fill` here, unlike the two drawing layers below — its
              // fixed `width: 100%` is not overridable and PHOTO_BOX needs to
              // state its own. See PHOTO_BOX.
              //
              ...
              style={
                {
                  // Held in from the frame's edges and rounded — see
                  // PHOTO_BOX. The two drawing layers below are deliberately
                  // *not* inset with it: they are linework on transparency
                  // and already carry their own margin, so insetting them too
                  // would read as the illustration shrinking rather than as
                  // the photograph being placed.
                  ...PHOTO_BOX,
                  "--photo-reveal": 0,
